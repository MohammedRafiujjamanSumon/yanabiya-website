  ...
  return (
    <footer className="border-t border-slate-200 bg-white mt-20">
      <div className="container-x py-14 grid gap-10 md:grid-cols-4">
        <div>
          <div className="font-serif text-xl text-slate-900 mb-2">Yanabiya Group</div>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-slate-600">
            {sections.slice(0, 7).map((l) => (
              <li key={l.id}><a href={`#${l.id}`} className="hover:text-brand-accent">{t(l.tKey)}</a></li>
            ))}
          </ul>
        </div>
        <div>
      ...
      <div className="border-t border-slate-200">
        <div className="container-x py-5 flex flex-col md:flex-row items-center justify-between text-xs text-slate-500">
